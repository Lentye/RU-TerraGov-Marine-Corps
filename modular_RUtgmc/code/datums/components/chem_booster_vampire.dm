...
/datum/component/chem_booster/vampire
		/list/datum/action/component_actions = list(
		...
		/datum/action/chem_booster/vampire/extractor = PROC_REF(extractor)
	)

/datum/action/chem_booster/vampire/extractor
	name = "Extract Valipire Chemical extractor"
	action_icon = 'icons/mob/actions.dmi'
	action_icon_state = "valipire_extractor"
	keybinding_signals = list(
		KEYBINDING_NORMAL = COMSIG_KB_VALI_EXTRACTOR,
	)
	///Records the last time the action was used to avoid accidentally cancelling the effect when spamming the button in-combat
	var/active

/datum/action/chem_booster/vampire/extractor/action_activate()
	. = ..()
	if(!.)
		return

/datum/action/chem_booster/vampire/extractor/proc/extractor()

/datum/component/chem_booster/vampire/extractor_offon(datum/source)
	SIGNAL_HANDLER
	INVOKE_ASYNC(src, PROC_REF(extractor), wearer)

/datum/component/chem_booster/vampire/extractor_offon(obj/item/weapon_to_connect)
	if(connected_weapon)
...
/datum/component/chem_booster/vampire/setup_reagent_info()
...
/datum/component/chem_booster/vampire/examine(datum/source, mob/user, list/examine_text)
	SIGNAL_HANDLER
...
/datum/component/chem_booster/vampire/dropped(datum/source, mob/user)
	SIGNAL_HANDLER
	if(!iscarbon(user))
		return
	if(boost_on)
...
/datum/component/chem_booster/vampire/equipped(datum/source, mob/equipper, slot)
	SIGNAL_HANDLER
...
/datum/component/chem_booster/vampire/process()
 ...
 */
/datum/component/chem_booster/vampire/configure(datum/source)
	SIGNAL_HANDLER
...
/datum/component/chem_booster/vampire/show_radial()
...
/datum/component/chem_booster/vampire/on_off(datum/source)
	SIGNAL_HANDLER
	if(!boost_on)
...
/datum/component/chem_booster/vampire/update_boost(amount)
	boost_amount = amount
...
/datum/component/chem_booster/vampire/setup_bonus_effects()
	if(!boost_on)
...
/datum/component/chem_booster/vampire/scan_user(datum/source)
	SIGNAL_HANDLER
	INVOKE_ASYNC(analyzer, TYPE_PROC_REF(/obj/item/healthanalyzer, attack), wearer, wearer, TRUE)

/datum/component/chem_booster/vampire/vali_connect(datum/source)
	SIGNAL_HANDLER
...
/datum/component/chem_booster/vampire/connect_weapon()
...
/datum/component/chem_booster/vampire/manage_weapon_connection(obj/item/weapon_to_connect)
	if(connected_weapon)
		wearer.balloon_alert(wearer, "Disconnected [connected_weapon]")
		REMOVE_TRAIT(connected_weapon, TRAIT_NODROP, VALI_TRAIT)
		UnregisterSignal(connected_weapon, COMSIG_ITEM_ATTACK)
...
/datum/component/chem_booster/vampire/drain_resource(datum/source, mob/living/M, mob/living/user)
	SIGNAL_HANDLER
	if(!isxeno(M))
		return
	if(M.stat == DEAD)
...
/datum/component/chem_booster/vampire/update_resource(amount)
...
/datum/component/chem_booster/vampire/extract(volume)
	if(wearer.do_actions)
		return
...
/datum/component/chem_booster/vampire/load_up()
	if(wearer.do_actions)
		return
...
/datum/component/chem_booster/vampire/get_meds_beaker_contents()
...
/datum/action/chem_booster/vampire/configure
...
/datum/action/chem_booster/vampire/power
...
/datum/action/chem_booster/vampire/power/action_activate()
...
/datum/action/chem_booster/vampire/power/update_onoff_icon()
	var/datum/component/chem_booster/vampire/target_component = target
...
/datum/action/chem_booster/vampire/connect_weapon
	name = "Connect Weapon"
	action_icon = 'icons/mob/actions.dmi'
